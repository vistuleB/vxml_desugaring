...
fn vxml_to_blamed_lines(
  ...
  case vxml {
    T(_, lines) -> {
      list.map(lines, fn(x){
        BlamedLine(x.blame, indent, x.content)
      })
    }
    V(blame, tag_name, blamed_attributes, children) -> {
      ...
      let children_blamed_lines = vxmls_to_blamed_lines_internal(children, indent + 1)
...
fn vxmls_to_blamed_lines_internal(vxmls: List(VXML), indent: Int) ->  List(BlamedLine) {
    case vxmls {
      [] -> []
      [first, ..rest] -> {
...
pub fn debug_pipeline(vxml: VXML, pipeline: Pipeline, step: Int, max_length: Int) -> String {
case pipeline {
      [] -> ""
      [#(desugarer_desc, desugarer_fun), ..rest] -> {

        let pipe_info = "// PIPELINE STEP " <> ins(step) <>"\n"
        ...
        case desugarer_fun(vxml) {
          Ok(vxml) -> {
            pipe_info
              <> header(max_length)
              <> vxml_to_blamed_lines(vxml, 0) |> pretty_print(pipeline_docs_annotator(max_length))
              <> footer(max_length)
          ...
          Error(error) -> {
            pipe_info
              <> "FAILED ON: " 
              <> error.blame.filename <> ":" <> ins(error.blame.line_no) <> "\n"
...
) ->  String
{
  ...
  let output = output 
                <> "// WLY -> VXML\n"
                <> header(max_length)
                <> vxmls_to_blamed_lines(vxmls) |> pretty_print(pipeline_docs_annotator(max_length))
