...
import infrastructure.{type Pipe, type DesugaringError } as infra
...
pub type ParsedSourceConverter(b) = fn(b) -> List(VXML)
...
pub type Splitter(d, e) = fn(VXML) -> Result(List(#(String, VXML, d)), e)
...
pub type Emitter(d, f) = fn(#(String, VXML, d)) -> Result(#(String, List(BlamedLine), d), f)
...
pub type Prettifier(d, g, h) = fn(String, #(String, d), g) -> Result(Nil, h)

pub type PrettifierDebugOptions(d, g) {
  PrettifierDebugOptions(
    debug_print: fn(String, d, g) -> Bool,
  )
}
...
pub fn prettier_prettifier(output_dir: String, pair: #(String, d), prettify: Bool) -> Result(Nil, #(Int, String)) {
...
pub type RendererError(a, c, e, f, h) {
  ...
  SplitterError(e)
  EmittingOrPrintingOrPrettifyingErrors(List(ThreePossibilities(f, simplifile.FileError, h)))
...
pub fn run_renderer(
    ...
    fn (result) {
      case result {
        ...
        Ok(#(local_path, blamed_lines, fragment_type)) -> {
          case debug_options.emitter_debug_options.debug_print(local_path, blamed_lines, fragment_type) {
            False -> Nil
            True -> {
              blamedlines.blamed_lines_to_table_vanilla_bob_and_jane_sue("(emitter_debug_options:" <> local_path <> ")", blamed_lines)
              |> io.println
            }
          }
        }
  ...
  let fragments = {
    ...
    |> list.map(
      fn (result) {
        case result {
          Error(error) -> {
            possible_error_message(debug_options, "emitting error: " <> ins(error))
            Error(C1(error))
          }
    ...
    fn (result) {
      case result {
        ...
        Ok(#(local_path, content, fragment_type)) -> {
          case debug_options.printer_debug_options.debug_print(local_path, fragment_type) {
            False -> Nil
            True -> {
              let header = "----------------- printer_debug_options: " <> local_path <> " -----------------"
              io.println(header)
              io.println(content)
  ...
  let fragments = 
    ...
    |> list.map(
      fn (result) {
        ...
        use output_dir <- infra.on_none_on_some(parameters.output_dir, Ok(#(local_path, fragment_type)))
  ...
  let fragments =
    ...
    |> list.map(
      fn (result) {
        ...
        case renderer.prettifier(output_dir, #(local_path, fragment_type), parameters.prettifying_option) {
          Error(e) -> Error(C3(e))
          Ok(Nil) -> {
            case debug_options.basic_messages {
              False -> Nil
  ...
  |> list.each(
    fn (result) {
      use #(local_path, fragment_type) <- infra.on_error_on_ok(result, fn(_) { Nil })
      use output_dir <- infra.on_none_on_some(parameters.output_dir, Nil)
      case debug_options.prettifier_debug_options.debug_print(local_path, fragment_type, parameters.prettifying_option) {
        False -> Nil
        True -> {
          let path = output_dir <> "/" <> local_path
...
fn take_strings_while_not_key(upcoming: List(String), bundled: List(String)) -> #(List(String), List(String)) {
  case upcoming {
...
pub fn double_dash_keys(arguments: List(String)) -> Result(List(#(String, List(String))), String) {
  case arguments {
    [] -> Ok([])
    [first, ..rest] -> {
      case string.starts_with(first, "--") {
...
fn amend_prettifying_option(amendment: CommandLineAmendments(g), val: g) -> CommandLineAmendments(g) {
  CommandLineAmendments(
    amendment.input_dir,
...
fn amend_debug_pipeline_range(amendment: CommandLineAmendments(a), start: Int, end: Int) -> CommandLineAmendments(a) {
  CommandLineAmendments(
    amendment.input_dir,
    amendment.output_dir,
...
fn amend_debug_pipeline_desugarer_names(amendment: CommandLineAmendments(a), names: List(String)) -> CommandLineAmendments(a) {
  CommandLineAmendments(
    amendment.input_dir,
    amendment.output_dir,
    amendment.prettifying_option,
...
pub fn amend_debug_blamed_lines_fragments_local_paths(amendment: CommandLineAmendments(a), names: List(String)) -> CommandLineAmendments(a) {
  CommandLineAmendments(
    amendment.input_dir,
    amendment.output_dir,
    amendment.prettifying_option,
...
fn amend_debug_printed_string_fragments_local_paths(amendment: CommandLineAmendments(a), names: List(String)) -> CommandLineAmendments(a) {
  CommandLineAmendments(
    amendment.input_dir,
    amendment.output_dir,
    amendment.prettifying_option,
...
fn amend_debug_prettified_string_fragments_local_paths(amendment: CommandLineAmendments(a), names: List(String)) -> CommandLineAmendments(a) {
  CommandLineAmendments(
    amendment.input_dir,
    amendment.output_dir,
    amendment.prettifying_option,
      ...
      case option {
        ...
        "--spotlight" -> {
          case shellout.command(
            ...
            Ok(_) -> Ok(amendment)
            Error(#(code, message)) -> {
              io.println("shellout error running spotlight: " <> ins(#(code, message)))
            ...
            True -> {
              ...
              case list.length(pieces) {
                2 -> {
                  ...
                  case int.parse(b), int.parse(c) {
                    Ok(debug_start), Ok(debug_end) -> {
                      Ok(amendment |> amend_debug_pipeline_range(debug_start, debug_end))
                    }
                ...
                1 -> {
                  ...
                  case int.parse(b) {
                    Ok(debug_start) -> {
                      Ok(amendment |> amend_debug_pipeline_range(debug_start, debug_start))
                    }
            ...
            False -> {
              case dict.get(prettier_options_dict, option) {
                ...
                Ok(prettifying_option) -> {
                  case values {
                    [] -> Ok(amendment |> amend_prettifying_option(prettifying_option))
...
fn pr_amend_input_dir(input_dir: String, amendments: CommandLineAmendments(g)) -> String {
...
fn pr_amend_output_dir(output_dir: Option(String), amendments: CommandLineAmendments(g)) -> Option(String) {
  case amendments.input_dir {
    None -> output_dir
...
fn pr_amend_prettifying_option(prettifying_option: g, amendments: CommandLineAmendments(g)) -> g {
...
pub fn db_amend_emitter_debug_options(
  ...
  EmitterDebugOptions(
    fn (local_path, lines, fragment_type) {
      ...
      list.contains(amendments.debug_blamed_lines_fragments_local_paths, local_path)
    },
    artifact_print,
    artifact_directory
  )
    ...
    fn (local_path, fragment_type) {
      {
        ...
        list.contains(amendments.debug_printed_string_fragments_local_paths, local_path)
      }
    }
  )
}
...
pub fn db_amend_prettifier_debug_options(
  ...
  let PrettifierDebugOptions(
    previous_condition
  ) = options

  PrettifierDebugOptions(
    fn (local_path, fragment_type, prettifying_enum) {
      {
        previous_condition(local_path, fragment_type, prettifying_enum) ||
        list.contains(amendments.debug_prettified_string_fragments_local_paths, local_path)
      }
    },
  )
}
...
pub fn amend_renderer_debug_options_by_command_line_amendment(
  ...
  RendererDebugOptions(
    ...
    db_amend_pipeline_debug_options(debug_options.pipeline_debug_options, amendments),
    debug_options.splitter_debug_options,
    db_amend_emitter_debug_options(debug_options.emitter_debug_options, amendments),
    db_amend_printed_debug_options(debug_options.printer_debug_options, amendments),
    db_amend_prettifier_debug_options(debug_options.prettifier_debug_options, amendments),
  )
}
...
pub fn empty_printer_debug_options(
) -> PrinterDebugOptions(d) {
  PrinterDebugOptions(
      debug_print: fn(_local_path, _fragment_type : d) { False },
  )
}
