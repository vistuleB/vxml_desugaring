...
fn param_transform(
  ...
  use parent, _ <- infra.on_lazy_empty_on_nonempty(
    ancestors,
    fn() { Ok(vxml) }
  )
  ...
  use pairs <- infra.on_error_on_ok(
  ...
  let attributes_to_add = list.fold(
    over: pairs,
    from: [],
    with: fn (so_far, pair) {
      let #(key, value) = pair
  ...
  Ok(
    V(
      blame,
      tag,
      list.append(attributes, attributes_to_add),
      children
    )
  )
}
...
fn transform_factory(
  param: Param,
) -> infra.NodeToNodeFancyTransform {
...
type Param = Dict(#(String, String), List(#(String, String)))

type Extra = List(#(String, String, String, String))
