    ...
    V(_, child_tag, attributes, _) if child_tag == inner.0 -> {
      ...
      let attributes =
        case list.contains(old_attribute_keys, inner.2) {
          ...
          False -> list.append(
            attributes,
            [BlamedAttribute(infra.blame_us("add_attribute_if_child_of"), inner.2, inner.3)],
          )
        }
...
const name = "add_attribute_if_child_of_no_list"
const constructor = add_attribute_if_child_of_no_list
...
/// exists
pub fn add_attribute_if_child_of_no_list(param: Param) -> Desugarer {
  Desugarer(
    name,
