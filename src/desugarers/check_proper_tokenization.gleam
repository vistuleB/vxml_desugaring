  ...
  case vxml {
    ...
    V(_, _, _, children) -> case has_href(vxml) || list.any(children, is_v_and_has_href) {
      False -> case list.any(children, is_v_and_has_tag_starting_with(_, "__")) {
        True -> Error(infra.DesugaringError(vxml.blame, "found tokenization error"))
        False -> Ok(vxml)
      }
      True -> case remaining_properly_tokenized(False, children) {
        True -> Ok(vxml)
        False -> Error(infra.DesugaringError(vxml.blame, "found tokenization error"))
      }
    }
  }
}
...
//------------------------------------------------53
/// 
pub fn check_proper_tokenization() -> Desugarer {
  Desugarer(
    name,
    option.None,
    option.None,
