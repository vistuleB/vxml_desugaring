...
fn extract_subchapter_title(chapter: VXML) -> List(#(SubChapterNo, SubchapterTitle)) {
  ...
  chapter
  |> infra.index_kind_children_with_tag("Sub")
...
fn construct_index(chapters: List(#(ChapterNo, ChapterTitle, List(#(SubChapterNo, SubchapterTitle))))) -> VXML {
  ...
  |> fn(list_node) {
    V(blame, "Index", [BlamedAttribute(blame, "class", "main-column-width")], [list_node])
  }
}

fn at_root(root: VXML) -> Result(VXML, DesugaringError) {
  let assert V(_, "Book", _attrs, _children) = root
  let index_node =
        root
