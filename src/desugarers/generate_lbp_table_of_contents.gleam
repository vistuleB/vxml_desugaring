...
fn the_desugarer(
  ...
  use bootcamp_menu_items <- infra.on_error_on_ok(
    over: {
        bootcamps
        |> list.index_map(
          fn(bootcamp : VXML, index) { chapter_link(chapter_link_component_name, bootcamp, index + 1) }
        )
        |> result.all
    },
    with_on_error: Error
  ...
  let bootcamps_div = div_with_id_title_and_menu_items(
    type_of_chapters_title_component_name,
    "bootcamp",
    "Bootcamps",
    bootcamp_menu_items
  ...
  let children = case list.is_empty(chapter_menu_items), list.is_empty(bootcamp_menu_items), maybe_spacer {
    True, True, _ -> []
